...
{
	assert (arr.length > 0);
	int[] OneMore = new int[arr.length + 1];
	for(int i = 0;i < arr.length; i++) {
		OneMore[i] = arr[i];
{	
		OneMore[OneMore.length - 1] = num;
		}
	}
	return OneMore;
}
...
{
	assert (arr.length >= 2);
	int[] OneLess = new int[arr.length-1];
	for(int i = 0; i < arr.length; i++) {
		if(i != idx && i < idx) {
			OneLess[i] = arr[i];
		}else {
...
//b. Carry out the following actions
	...
	int appendNum = 200;
	int[] OneMore = append(a1,appendNum);
	System.out.println(Arrays.toString(OneMore));
	
	int removeIdx = 5;
	int[] OneLess = remove(a2, removeIdx);
	System.out.println(Arrays.toString(OneLess));
	
	int sumOfEven = sumEven(OneMore);
